"""
STABLE-MERGE (ICML Idea_2) — Extension utilities (ICML-grade evaluation)
FULL WORKING SCRIPT (Colab/Notebook friendly) + TG4H/NHANES fix

สิ่งที่ “ฉบับสมบูรณ์” นี้แก้ให้คุณแล้ว:
1) ไม่ต้องพึ่ง sys.argv ใน Colab: เรียก main([...]) ได้เลย
2) TG4H ใน NHANES เป็น all-missing -> auto เลือกคอลัมน์ที่ใช้ได้จริงจาก [TG4h, TG4H, TG]
3) shared threshold:
   - ปกติเอา threshold จาก synthetic (quantile)
   - ถ้าเอาไปใช้กับ NHANES แล้วเหลือคลาสเดียว -> สลับไปใช้ threshold จาก NHANES (quantile)
   - แล้ว “รีเลเบล” ทั้งสอง env ด้วย threshold เดียวกัน (เพื่อเทียบกันได้)
4) กัน error พวก 0 sample / label คลาสเดียว / คอลัมน์ numeric แปลงไม่ได้
5) ได้ outputs แบบเดียวกับสคริปต์ extension เดิม:
   - reports/per_env_metrics.csv
   - reports/model_table_aggregated.csv
   - reports/selection_outcomes.csv
   - reports/accuracy_vs_stability_table.csv
   - reports/ranking_stability_summary.json
   - figures/pairwise_flip_heatmap.png
   - figures/model_flip_involvement.png

รองรับ task:
- mimic_mortality (ต้องมี demo + full)
- tg_high_response (ต้องมี synthetic + nhanes)

Usage ใน Colab:
  # TG
  main([
    "--task","tg_high_response",
    "--outdir","runs/ext_tg",
...
def _pick_default_path(prefer_content: str, fallback_mnt: str) -> str:
    if Path(prefer_content).exists():
        return prefer_content
    if Path(fallback_mnt).exists():
        return fallback_mnt
    return prefer_content  # keep a sane-looking default even if missing

DEFAULT_PATHS = {
    "mimic_demo": _pick_default_path(
        "/content/demo_analytic_dataset_mortality_all_admissions.csv",
        "/mnt/data/demo_analytic_dataset_mortality_all_admissions.csv",
    ),
    "mimic_full": _pick_default_path(
        "/content/full_analytic_dataset_mortality_all_admissions.csv",
        "/mnt/data/full_analytic_dataset_mortality_all_admissions.csv",
    ),
    "synthetic": _pick_default_path(
        "/content/Synthetic_Dataset_1500_Patients_precise.csv",
        "/mnt/data/Synthetic_Dataset_1500_Patients_precise.csv",
    ),
    "nhanes": _pick_default_path(
        "/content/nhanes_rsce_dataset_clean.csv",
        "/mnt/data/nhanes_rsce_dataset_clean.csv",
    ),
}
...
def _require_files(paths: Dict[str, str], keys: List[str]) -> bool:
    ...
    if missing:
        ...
        for k, fp in missing:
            print(f"  - {k}: {fp}")
        print("\n✅ วิธีแก้ใน Colab:")
        print("  1) อัปโหลดไฟล์เข้า Colab (sidebar > Files > Upload)")
        print("  2) แล้วส่ง path ให้ main([...]) หรือแก้ DEFAULT_PATHS ให้ตรง")
        return False
    return True
...
def compute_metrics(y_true: np.ndarray, y_prob: np.ndarray) -> MetricBundle:
    ...
    if len(y_true) == 0:
        raise ValueError("compute_metrics: empty y_true (0 samples)")

    # roc_auc needs 2 classes
    if np.unique(y_true).size < 2:
        raise ValueError(f"compute_metrics: y_true has only one class: {np.unique(y_true)}")
...
def constrained_selection(
    ...
    if len(feasible) > 0:
        ...
        return SelectionOutcome(
            ...
            details={
                ece_col: float(row[ece_col]),
                flip_col: float(row[flip_col]),
                "tau": float(tau),
                "kappa": float(kappa),
            },
        )
...
def train_oof_predictions(
    ...
    if len(y) == 0:
        raise ValueError("train_oof_predictions: 0 samples after preprocessing")

    if np.unique(y).size < 2:
        vc = pd.Series(y).value_counts().to_dict()
        raise ValueError(f"train_oof_predictions: target has only one class: {vc}")
    ...
    # Drop all-NaN columns (กันคอลัมน์ที่พังทั้งแท่ง)
    ...
    for fold, (tr, te) in enumerate(tqdm(list(cv.split(X_df, y)), desc="CV folds", total=n_splits)):
...
def _choose_numeric_col(df: pd.DataFrame, candidates: List[str], env_name: str) -> Tuple[str, pd.Series]:
    """
    เลือกคอลัมน์ที่ 'มีค่าจริง' หลัง pd.to_numeric(errors='coerce') อย่างน้อย 1 ค่า
    """
    ...
    if not cols_present:
        raise ValueError(f"{env_name}: None of candidate columns exist: {candidates}")
    raise ValueError(f"{env_name}: columns exist but all missing after numeric coercion: {cols_present}")


def _label_from_threshold(df: pd.DataFrame, numeric_series: pd.Series, threshold: float, label_name: str = "label") -> pd.DataFrame:
    s = numeric_series.copy()
    s = s.clip(lower=0)  # กันค่าติดลบ
    out = df.loc[s.notna()].copy()
    s = s.loc[s.notna()]
    out[label_name] = (s >= float(threshold)).astype(int).values
    return out


def add_tg_label_with_shared_threshold(
    synth: pd.DataFrame,
    nhanes: pd.DataFrame,
    quantile: float = 0.75,
    ...
    """
    สร้าง label (binary) สำหรับ TG task โดยพยายามใช้ shared threshold เดียวกัน

    ขั้นตอน:
    1) เลือกคอลัมน์ TG ที่ใช้ได้จริงในแต่ละ env จาก candidates
    2) คำนวณ thr_synth = quantile ของ synthetic
    3) ลอง label NHANES ด้วย thr_synth:
       - ถ้าคลาสเดียว -> สลับไป thr_nhanes = quantile ของ NHANES
       - แล้วใช้ thr_nhanes เป็น shared threshold ทั้งสอง env (รีเลเบล synthetic ด้วย)
    """
    if candidates is None:
        candidates = ["TG4h", "TG4H", "TG"]

    synth_col, synth_s = _choose_numeric_col(synth, candidates, "SYNTHETIC")
    nh_col, nh_s = _choose_numeric_col(nhanes, candidates, "NHANES")

    thr_from_synth = float(synth_s.dropna().clip(lower=0).quantile(quantile))
    thr_from_nhanes = float(nh_s.dropna().clip(lower=0).quantile(quantile))

    # try synth threshold on both
    synth_l = _label_from_threshold(synth, synth_s, thr_from_synth, label_name="label")
    nh_l_try = _label_from_threshold(nhanes, nh_s, thr_from_synth, label_name="label")

    switched = False
    shared_thr = thr_from_synth

    if nh_l_try["label"].nunique() < 2:
        # fallback: use NHANES quantile threshold
        switched = True
        shared_thr = thr_from_nhanes
        nh_l = _label_from_threshold(nhanes, nh_s, shared_thr, label_name="label")
        synth_l = _label_from_threshold(synth, synth_s, shared_thr, label_name="label")
    else:
        nh_l = nh_l_try

    # final safety
    for name, d in [("SYNTHETIC", synth_l), ("NHANES", nh_l)]:
        if len(d) == 0:
            raise ValueError(f"{name}: 0 samples after labeling (all NaN?)")
        if d["label"].nunique() < 2:
            vc = d["label"].value_counts().to_dict()
            raise ValueError(f"{name}: label เหลือคลาสเดียว: {vc} (threshold={shared_thr})")

    meta = {
        "synth_col_used": synth_col,
        "nhanes_col_used": nh_col,
        "quantile": float(quantile),
        "thr_from_synth_quantile": float(thr_from_synth),
        "thr_from_nhanes_quantile": float(thr_from_nhanes),
        "shared_threshold_used": float(shared_thr),
        "switched_to_nhanes_threshold": bool(switched),
        "synth_label_counts": {str(k): int(v) for k, v in synth_l["label"].value_counts().to_dict().items()},
        "nhanes_label_counts": {str(k): int(v) for k, v in nh_l["label"].value_counts().to_dict().items()},
    }
    return synth_l.reset_index(drop=True), nh_l.reset_index(drop=True), meta
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    if args.task == "mimic_mortality":
        ...
        synth_l, nh_l, label_meta = add_tg_label_with_shared_threshold(
            synth=synth,
            nhanes=nh,
            quantile=float(args.positive_quantile),
            candidates=["TG4h", "TG4H", "TG"],
        )

        # ทำ common columns แบบเดิม (ให้เหลือเฉพาะ feature ที่ตรงกัน)
        ...
        # ให้ order เป็น synthetic ก่อน เพื่อให้ ref_env default = synthetic
        env_dfs = {"synthetic": synth_l, "nhanes": nh_l}
        target_col = "label"

        save_json(label_meta, outdir / "reports" / "label_definition.json")
    ...
    # ---------- Console summary ----------
    if args.task == "tg_high_response" and label_meta is not None:
        print("\n=== TG HIGH RESPONSE (FIXED) ===")
        print("Label meta:", json.dumps(label_meta, indent=2))
        print("\nPer-env metrics head:")
...
if __name__ == "__main__":
    # ถ้ารันใน notebook แล้วไม่ส่ง argv -> ใช้ COLAB_DEFAULT_ARGS
    main([
    "--task", "tg_high_response",
    "--outdir", "runs/ext_tg",
    "--synthetic", "/content/Synthetic_Dataset_1500_Patients_precise.csv",
